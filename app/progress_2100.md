...
| `POST` | `/results` | Validates images + profile; runs model; stores payload in memory cache; **303 redirect** to `/results/view?rid=…`. Uses `response_model=None` because the handler returns either `RedirectResponse` or an HTML error page. |
...
1. **Tabbed home UI** — **Images** tab (up to five file inputs, same field name `images`) and **Customer profile** tab (form fields + optional CSV upload).
...
3. **Profile schema** — Loaded from `app/profile_attributes.json`:
   ...
   - **Numerical** attributes: free numeric input, **non-negative** enforced in UI (`min="0"`) and server-side (`validate_profile`).
   - **Categorical** attributes: one dropdown per **grouped** manifest feature (`type__attribute_name`); each option is a **`value` + `label`** pair (`value` = full multihot token; `label` = arbitrary display string editable in JSON).
   - **Legacy support:** categorical `options` may still be a list of strings; labels are auto-derived.
4. **CSV profile** — Optional file overrides the form. Headers must match attribute `id` list **exactly** (order-sensitive). One data row; UTF-8; numerical cells non-negative; categorical cells must match a known **`value`** string.
...
10. **Stub model** — `model_service.stub_predict`: deterministic scores from hashes + profile digest; returns `slot_index`, `image_attributes` dict, and `reason` per image (replace with real trained pipeline later).
...
| `services/submission.py` | Multipart image handling; CSV parse; profile merge from form; `data_url_for_image`. |
...
| `services/model_service.py` | Stub prediction + placeholder per-image attributes. |
...
3. **Customer profile tab**  
   ...
   - **Optional CSV:** upload a one-row file whose headers match `profile_attributes.json` ids **in order**; if present, CSV **replaces** form values.

4. **Predict** — Submits the form; on success you are redirected to the **results** page.

5. **Results page** — Inspect rank order; click a thumbnail to load **attributes + reasoning** below. Use **← New prediction** to start over.

6. **Templates / samples** — Use **“this template”** on the form or `/profile/csv-template` for a fresh CSV shell; use `app/sample_valid_profile.csv` as a known-good upload example.
...
- Swap `stub_predict` in `services/model_service.py` for the real artifact; keep return shape compatible with `routers/web.py` (per-slot filename, score, reason, image-side attributes).
