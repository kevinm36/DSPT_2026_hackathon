"""Canonical IAB Tier 1 category list and the categorization prompt template.
...
The on-disk file ``IAB-t1.csv`` is one category per line (CSV-quoted when the
...
DEFAULT_CATEGORIES_PATH: Path = Path(__file__).parent / "IAB-t1.csv"

PROMPT_INSTRUCTION: str = (
    "Assign IAB tier1 categories to the image. Return a list of categories "
...
def load_categories(path: Path = DEFAULT_CATEGORIES_PATH) -> list[str]:
    """Return the canonical IAB Tier 1 category list, in file order.
